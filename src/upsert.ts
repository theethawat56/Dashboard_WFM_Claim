...
function getProductInfo(task: Task): any | null {
  const detail = task.detail;
  if (!detail) return null;
  const info = (detail as any).productInfo ?? (detail as any).product_info;
  return info ?? null;
}
...
function getIssueGroup(task: Task): string {
  const info = getProductInfo(task);
  const value = (info?.issueGroup as string | undefined) ?? "";
  return value || "";
}
...
function getClaimType(task: Task, taskType: "repair" | "claim"): string {
  ...
  const info = getProductInfo(task);
  const value = (info?.claimType as string | undefined) ?? "";
  return value || "";
}
...
export async function upsertTasks(
  ...
  for (const task of tasks) {
    ...
    const updatedTs =
      task.updated_timestamp ?? task.updatedTimestamp ?? null;
    const parentIds = getParentIds(task);
    const refNumbers = getRefNumbers(task);
    ...
    const detail = task.detail;
    const customerName =
      detail?.customer_name ?? detail?.customerName ?? null;
    const customerPhone =
      detail?.customer_phone ?? detail?.customerPhone ?? null;
    const customerProvince =
      detail?.customer_province ?? detail?.customerProvince ?? null;
    const shippingOption =
      detail?.shipping_option ?? detail?.shippingOption ?? null;
    const createDate =
      detail?.create_date ?? detail?.createDate ?? null;
